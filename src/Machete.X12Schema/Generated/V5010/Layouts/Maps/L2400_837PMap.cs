        ...
        X12LayoutMap<L2400_837P, X12Entity>
    {
        public L2400_837PMap()
        {
            ...
            Segment(x => x.LineSupplementalInformation, 3,
                x => x.Condition = parser => parser.Where(p => p.AttachmentTransmissionCode.IsEqualTo("AA") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("BM") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("EL") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("EM") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("FT") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("FX")));
            Segment(x => x.DurableMedicalEquipmentCertificateOfMedicalNecessityIndicator, 4,
                x => x.Condition = parser => parser.Where(p => p.AttachmentTransmissionCode.IsEqualTo("AB") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("AD") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("AF") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("AG") ||
                                                               p.AttachmentTransmissionCode.IsEqualTo("NS")));
            Segment(x => x.AmbulanceTransportInformation, 5);
            Segment(x => x.DurableMedicalEqumentCertification, 6);
            Segment(x => x.AmbulanceCertification, 7, x => x.Condition = parser => parser.Where(p => p.CodeCategory.IsEqualTo("07")));
            Segment(x => x.HospiceEmployeeIndicator, 8, x => x.Condition = parser => parser.Where(p => p.CodeCategory.IsEqualTo("70")));
            Segment(x => x.ConditionIndicatorDurableMedicalEquipment, 9, x => x.Condition = parser => parser.Where(p => p.CodeCategory.IsEqualTo("09")));
            ...
            Segment(x => x.MammographyCerficationNumber, 28,
            ...
            Segment(x => x.LineNote, 36,
                x => x.Condition = parser => parser.Where(p => p.NoteReferenceCode.IsEqualTo("ADD") || p.NoteReferenceCode.IsEqualTo("DCP")));
            ...
            Layout(x => x.SerivceFacilityLocation, 43);
