namespace Machete.X12Schema.V5010.Maps
{
        ...
        X12LayoutMap<L2300_837P, X12Entity>
    {
        public L2300_837PMap()
        {
            ...
            Segment(x => x.RepricerReceivedDate, 16, x => x.Condition = parser => parser.Where(p => p.DateTimeQualifier.IsEqualTo("050")));
            ...
            Segment(x => x.HomeboundIndicator, 40, x => x.Condition = parser => parser.Where(p => p.CodeCategory.IsEqualTo("75")));
            Segment(x => x.EPSDTReferral, 41, x => x.Condition = parser => parser.Where(p => p.CodeCategory.IsEqualTo("ZZ")));
            ...
            Segment(x => x.AnesthesiaRelatedProcedure, 43,
                x => x.Condition = parser => parser.Where(p => p.HealthCareCodeInformation1.Select(s => s.DiagnosisTypeCode).IsEqualTo("BP")));
            Segment(x => x.ConditionInfo, 44,
                x => x.Condition = parser => parser.Where(p => p.HealthCareCodeInformation1.Select(s => s.DiagnosisTypeCode).IsEqualTo("BG")));
