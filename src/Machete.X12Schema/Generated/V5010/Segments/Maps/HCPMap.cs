namespace Machete.X12Schema.V5010.Segments.Maps
{
    ...
    public class HCPMap : X12SegmentMap<HCP, X12Entity>
    {
        public HCPMap()
        {
            Id = "HCP";
            ...
            Value(x => x.RepricingOrganizationIdentifier, 4, x => x.MinLength(1).MaxLength(30));
            Value(x => x.RepricingPerDiem, 5, x => x.MinLength(1).MaxLength(9));
            Value(x => x.RepricedApprovedDrgCode, 6, x => x.MinLength(1).MaxLength(30));
            Value(x => x.RepricedApprovedAmount, 7, x => x.MinLength(1).MaxLength(18));
