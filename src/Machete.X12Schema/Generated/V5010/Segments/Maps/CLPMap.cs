namespace Machete.X12Schema.V5010.Segments.Maps
{
    ...
    public class CLPMap : X12SegmentMap<CLP, X12Entity>
    {
        public CLPMap()
        {
            Id = "CLP";
            ...
            Value(x => x.TotalClaimCharge, 3, x => x.MinLength(1).IsRequired());
            Value(x => x.TotalClaimPayment, 4, x => x.MinLength(1).IsRequired());
            Value(x => x.PatientResponsibility, 5, x => x.MinLength(1));
            ...
            Value(x => x.CLP10, 10, x => x.MinLength(1));
            Value(x => x.DrgCode, 11, x => x.MinLength(1).MaxLength(4));
            Value(x => x.DrgWeight, 12, x => x.MinLength(1));
            Value(x => x.DischargeFraction, 13, x => x.MinLength(1));
        }
    }
}