namespace Machete.X12Schema.V5010.Maps
{
    ...
    public class T860Map :
        X12LayoutMap<T860, X12Entity>
    {
        public T860Map()
        {
            Id = "T860";
            Name = "860 Purchase Order";
            ...
            Segment(x => x.RestrictionsOrConditions, 26);
            Segment(x => x.MarksAndNumbersInformation, 27);
            Segment(x => x.MaintenanceType, 28);
            Segment(x => x.TaxInformation, 29);
            Segment(x => x.PercentAmounts, 30);
            Layout(x => x.LoopLDT, 31);
            Layout(x => x.LoopAMT, 32);
            Layout(x => x.LoopN9, 33);
            Layout(x => x.LoopN1, 34);
            Layout(x => x.LoopLM, 35);
            Layout(x => x.LoopSPI, 36);
            Layout(x => x.LoopADV, 37);
            Layout(x => x.LoopPOC, 38);
            Layout(x => x.LoopCTT, 39);
            Segment(x => x.TransactionSetTrailer, 40);
            Segment(x => x.FunctionalGroupTrailer, 41);
        }
    }
}