...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7SegmentMap<STF>
    {
        public STFMap()
        {
            ...
            Value(x => x.DateTimeOfBirth, 6, x => {x.Format = "LONGDATETIME6";});
            ...
            Value(x => x.AutoInsExpires, 24, x => {x.Format = "LONGDATE24";});
            Value(x => x.DateLastDMVReview, 25, x => {x.Format = "LONGDATE25";});
            Value(x => x.DateNextDMVReview, 26, x => {x.Format = "LONGDATE26";});
            Entity(x => x.Race, 27);
            Entity(x => x.EthnicGroup, 28);
            Value(x => x.ReActivationApprovalIndicator, 29);
            Entity(x => x.Citizenship, 30);
            Value(x => x.DateTimeOfDeath, 31, x => {x.Format = "LONGDATE31";});
            Value(x => x.DeathIndicator, 32);
            Entity(x => x.InstitutionRelationshipTypeCode, 33);
            Entity(x => x.InstitutionRelationshipPeriod, 34);
            Value(x => x.ExpectedReturnDate, 35, x => {x.Format = "LONGDATE35";});
