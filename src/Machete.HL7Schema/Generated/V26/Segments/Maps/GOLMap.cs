...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7SegmentMap<GOL>
    {
        public GOLMap()
        {
            ...
            Value(x => x.ActionDateTime, 2, x => {x.Format = "LONGDATETIME2";x.Required = true;});
            ...
            Value(x => x.GoalEstablishedDateTime, 7, x => {x.Format = "LONGDATETIME7";});
            Value(x => x.ExpectedGoalAchieveDateTime, 8, x => {x.Format = "LONGDATETIME8";});
            Entity(x => x.GoalClassification, 9);
            Entity(x => x.GoalManagementDiscipline, 10);
            Entity(x => x.CurrentGoalReviewStatus, 11);
            Value(x => x.CurrentGoalReviewDateTime, 12, x => {x.Format = "LONGDATETIME12";});
            Value(x => x.NextGoalReviewDateTime, 13, x => {x.Format = "LONGDATETIME13";});
            Value(x => x.PreviouGoalReviewDateTime, 14, x => {x.Format = "LONGDATETIME14";});
            ...
            Value(x => x.GoalLifeCycleStatusDateTime, 19, x => {x.Format = "LONGDATETIME19";});
