...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7SegmentMap<PR1>
    {
        public PR1Map()
        {
            ...
            Value(x => x.ProcedureCodingMethod, 2, x => {x.Format = "LONGDATETIME2";});
            Entity(x => x.ProcedureCode, 3, x => {x.Required = true;});
            Value(x => x.ProcedureDescription, 4, x => {x.Format = "LONGDATETIME4";});
            Value(x => x.ProcedureDateTime, 5, x => {x.Format = "LONGDATETIME5";x.Required = true;});
            Value(x => x.ProcedureFunctionalType, 6);
            Value(x => x.ProcedureMinutes, 7);
            Value(x => x.Anesthesiologist, 8, x => {x.Format = "LONGDATETIME8";});
            Value(x => x.AnesthesiaCode, 9);
            Value(x => x.AnesthesiaMinutes, 10);
            Value(x => x.Surgeon, 11, x => {x.Format = "LONGDATETIME11";});
            Value(x => x.ProcedurePractitioner, 12, x => {x.Format = "LONGDATETIME12";});
