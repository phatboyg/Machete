...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7SegmentMap<OM7>
    {
        public OM7Map()
        {
            ...
            Entity(x => x.UniversalServiceIdentifier, 2, x => {x.Required = true;});
            Entity(x => x.CategoryIdentifier, 3);
            Value(x => x.CategoryDescription, 4);
            Value(x => x.CategorySynonym, 5);
            Value(x => x.EffectiveTestServiceStartDateTime, 6, x => {x.Format = "LONGDATETIME6";});
            Value(x => x.EffectiveTestServiceEndDateTime, 7, x => {x.Format = "LONGDATETIME7";});
            ...
            Value(x => x.ConsentEffectiveStartDateTime, 13, x => {x.Format = "LONGDATETIME13";});
            Value(x => x.ConsentEffectiveEndDateTime, 14, x => {x.Format = "LONGDATETIME14";});
            ...
            Value(x => x.EffectiveDateTimeOfChange, 19, x => {x.Format = "LONGDATETIME19";});
