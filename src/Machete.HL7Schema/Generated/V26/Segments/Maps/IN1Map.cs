...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7SegmentMap<IN1>
    {
        public IN1Map()
        {
            ...
            Value(x => x.PlanEffectiveDate, 12, x => {x.Format = "LONGDATE12";});
            Value(x => x.PlanExpirationDate, 13, x => {x.Format = "LONGDATE13";});
            ...
            Value(x => x.InsuredSDateOfBirth, 18, x => {x.Format = "LONGDATETIME18";});
            ...
            Value(x => x.NoticeOfAdmissionDate, 24, x => {x.Format = "LONGDATE24";});
            Value(x => x.ReportOfEligibilityFlag, 25);
            Value(x => x.ReportOfEligibilityDate, 26, x => {x.Format = "LONGDATE26";});
            Value(x => x.ReleaseInformationCode, 27);
            Value(x => x.PreAdmitCert, 28);
            Value(x => x.VerificationDateTime, 29, x => {x.Format = "LONGDATETIME29";});
            ...
            Value(x => x.PolicyLimitAmount, 38, x => {x.Format = "LONGDATETIME38";});
            Value(x => x.PolicyLimitDays, 39);
            Value(x => x.RoomRateSemiPrivate, 40, x => {x.Format = "LONGDATETIME40";});
            Value(x => x.RoomRatePrivate, 41, x => {x.Format = "LONGDATETIME41";});
            ...
            Value(x => x.SignatureCodeDate, 51, x => {x.Format = "LONGDATE51";});
