...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7SegmentMap<PD1>
    {
        public PD1Map()
        {
            ...
            Value(x => x.ProtectionIndicatorEffectiveDate, 13, x => {x.Format = "LONGDATETIME13";});
            Entity(x => x.PlaceOfWorship, 14);
            Entity(x => x.AdvanceDirectiveCode, 15);
            Value(x => x.ImmunizationRegistryStatus, 16);
            Value(x => x.ImmunizationRegistryStatusEffectiveDate, 17, x => {x.Format = "LONGDATE17";});
            Value(x => x.PublicityCodeEffectiveDate, 18, x => {x.Format = "LONGDATE18";});
            Value(x => x.MilitaryBranch, 19);
            Value(x => x.MilitaryRankGrade, 20);
            Value(x => x.MilitaryStatus, 21);
            Value(x => x.AdvanceDirectiveLastVerifiedDate, 22, x => {x.Format = "LONGDATE22";});
        }
    }
}