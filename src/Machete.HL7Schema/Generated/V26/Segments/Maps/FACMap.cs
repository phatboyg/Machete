...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7V26SegmentMap<FAC>
    {
        public FACMap()
        {
            ...
            Entity(x => x.FacilityId, 1, x => {x.Required = true;});
            Value(x => x.FacilityType, 2);
            Entity(x => x.FacilityAddress, 3, x => {x.Required = true;});
            Entity(x => x.FacilityTelecommunication, 4, x => {x.Required = true;});
            Entity(x => x.ContactPerson, 5);
            Value(x => x.ContactTitle, 6);
            Entity(x => x.ContactAddress, 7);
            Entity(x => x.ContactTelecommunication, 8);
            Entity(x => x.SignatureAuthority, 9, x => {x.Required = true;});
