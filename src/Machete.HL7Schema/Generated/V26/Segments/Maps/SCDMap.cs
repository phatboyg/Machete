...
namespace Machete.HL7Schema.V26.Maps
{
        ...
        HL7SegmentMap<SCD>
    {
        public SCDMap()
        {
            ...
            Value(x => x.CycleStartTime, 1, x => {x.Format = "LONGDATETIME1";});
            ...
            Value(x => x.CycleStartDateTime, 11, x => {x.Format = "LONGDATETIME11";});
            Entity(x => x.DryTime, 12);
            Entity(x => x.LeakRate, 13);
            Entity(x => x.ControlTemperature, 14);
            Entity(x => x.SterilizerTemperature, 15);
            Value(x => x.CycleCompleteTime, 16, x => {x.Format = "LONGDATETIME16";});
