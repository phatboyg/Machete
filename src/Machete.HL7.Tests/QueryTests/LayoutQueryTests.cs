...
NTE|2||dsa";
            ...
            var pidSegment = result.Result.PID;
            
            Assert.IsTrue(pidSegment.HasValue);
            
            var patientId1 = pidSegment.Select(x => x.PatientIdentifierList)[0];
            var patientId2 = pidSegment.Select(x => x.PatientIdentifierList)[1];
            
            Assert.IsTrue(patientId1.HasValue);
            Assert.IsTrue(patientId1.IsPresent);
            Assert.IsTrue(patientId2.HasValue);
            Assert.IsTrue(patientId2.IsPresent);
            
            Assert.AreEqual("60043", patientId1.Select(x => x.IdNumber).ValueOrDefault());
            Assert.AreEqual("MACHETE", patientId1.Select(x => x.AssigningAuthority).Select(x => x.NamespaceId).ValueOrDefault());
            Assert.AreEqual("MRN", patientId1.Select(x => x.IdentifierTypeCode).ValueOrDefault());
            Assert.AreEqual("60044", patientId2.Select(x => x.IdNumber).ValueOrDefault());
            Assert.AreEqual("MACHETE", patientId2.Select(x => x.AssigningAuthority).Select(x => x.NamespaceId).ValueOrDefault());
            Assert.AreEqual("MRN", patientId2.Select(x => x.IdentifierTypeCode).ValueOrDefault());
        }
