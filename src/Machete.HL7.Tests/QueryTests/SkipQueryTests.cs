...
NTE|2||dsa";
        ...
        [Test]
        public void Should_be_able_to_skip_middle_segments_via_selection_using_SkipUntil()
        {
            const string message = @"MSH|^~\&|MACHETELAB|^DOSC|MACHETE|18779|20130405125146269||ORM^O01|1999077678|P|2.3|||AL|AL
NTE|1||KOPASD
NTE|2||A3RJ
...
NTE|2||dsa";
            ...
            var query = entityResult.Query(q =>
            {
                ...
                return from msh in q.Select<MSH>()
                    from notes in q.Select<NTE>().ZeroOrMore()
                    from pid in q.Select<PID>()
                    from ignored in q.Select<HL7Segment>().Skip(q.Select<ORC>())
                    from tests in testQuery.ZeroOrMore()
                    select new
                    {
                        MSH = msh,
        ...
        [Test]
        public void Should_be_able_to_skip_over_all_segments_using_SkipUntil()
        {
            const string message = @"MSH|^~\&|MACHETELAB|^DOSC|MACHETE|18779|20130405125146269||ORM^O01|1999077678|P|2.3|||AL|AL
NTE|1||KOPASD
NTE|2||A3RJ
