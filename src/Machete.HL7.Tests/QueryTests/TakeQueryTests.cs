namespace Machete.HL7.Tests.QueryTests
{
    ...
    [TestFixture]
    public class TakeQueryTests :
        HL7MacheteTestHarness<TestHL7Entity, HL7Entity>
    {
        [Test]
        public void Should_be_able_to_take_all_segments()
        {
...
NTE|2|dsa";
            ...
            var query = entityResult.Query(q =>
                ...
                from insurance in q.Select<IN1Segment>().Take(2)
                select insurance);
            
            Assert.IsTrue(query.HasResult);
            Assert.AreEqual(2, query.Result.Count);
            Assert.AreEqual("074394", query.Result[0].GroupNumber.ValueOrDefault());
            Assert.AreEqual("074395", query.Result[1].GroupNumber.ValueOrDefault());
        }
        
        [Test]
        public void Should_be_able_to_take_zero_segments()
        {
            const string message = @"MSH|^~\&|MACHETELAB|^DOSC|MACHETE|18779|20130405125146269||ORM^O01|1999077678|P|2.3|||AL|AL
PID|1|000000000026|60043^^^MACHETE^MRN||MACHETE^JOE||19890909|F|||123 SEASAME STREET^^Oakland^CA^94600||5101234567|5101234567||||||||||||||||N
IN1|1|||MACHETE INC|1234 Fruitvale ave^^Oakland^CA^94601^USA||5101234567^^^^^510^1234567|074394|||||||A1|MACHETE^JOE||19890909|123 SEASAME STREET^^Oakland^CA^94600||||||||||||N|||||666889999|0||||||F||||T||60043^^^MACHETE^MRN
...
NTE|2|dsa";
            ...
            var query = entityResult.Query(q =>
                from msh in q.Select<MSHSegment>()
                from pid in q.Select<PIDSegment>()
                from insurance in q.Select<IN1Segment>().Take(0)
                select insurance);
            
            Assert.IsTrue(query.HasResult);
            Assert.AreEqual(0, query.Result.Count);
        }
        
        [Test]
        public void Should_throw_ArgumentOutOfRangeException_when_trying_to_take_negative_count_of_segments()
        {
            const string message = @"MSH|^~\&|MACHETELAB|^DOSC|MACHETE|18779|20130405125146269||ORM^O01|1999077678|P|2.3|||AL|AL
PID|1|000000000026|60043^^^MACHETE^MRN||MACHETE^JOE||19890909|F|||123 SEASAME STREET^^Oakland^CA^94600||5101234567|5101234567||||||||||||||||N
IN1|1|||MACHETE INC|1234 Fruitvale ave^^Oakland^CA^94601^USA||5101234567^^^^^510^1234567|074394|||||||A1|MACHETE^JOE||19890909|123 SEASAME STREET^^Oakland^CA^94600||||||||||||N|||||666889999|0||||||F||||T||60043^^^MACHETE^MRN
...
NTE|2|dsa";
            ...
            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                var query = entityResult.Query(q =>
                    from msh in q.Select<MSHSegment>()
                    from pid in q.Select<PIDSegment>()
                    from insurance in q.Select<IN1Segment>().Take(-1)
                    select insurance);
            });
        }
    }
