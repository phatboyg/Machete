namespace Machete.HL7.Tests.EntityFieldAccessTests
{
    ...
    [TestFixture]
    public class SegmentListExtensionsTests :
...
OBR|5|PRO2350||11638^Urinalysis, with Culture if Indicated^L|||20130405135133||||N|||||92383^Machete^Janice|||||||||||^^^^^R";
            ...
            var obrs = notes.ToList()[0].Select(x => x.OBR).ToEnumerable();
            
            foreach (var obr in obrs)
            {
                int setId = obr.Select(x => x.SetId).ValueOrDefault();
                
                Assert.That(setId, Is.EqualTo(1).Or.EqualTo(2).Or.EqualTo(3));
            }
