...
OBR|1|PRO2350||11636^Urinalysis, with Culture if Indicated^L|||20130405135133";
        ...
        [Test]
        public async Task Should_be_able_to_format_advanced_query()
        {
            const string message = @"MSH|^~\&|MACHETELAB|^DOSC|MACHETE|18779|20130405125146269||ORM^O01|1999077678|P|2.3|||AL|AL
NTE|1||KOPASD
NTE|2||A3RJ
...
//
//            Parser<HL7Entity, ORM_O01> query = entityResult.CreateQuery(layout);
//
//            Result<Cursor<HL7Entity>, ORM_O01> result = entityResult.Query(query);
//
//            StringFormatResult<HL7Entity> orders = await Formatter.FormatToStringAsync(entityResult);
//            
//            Console.WriteLine(orders.Text);
        }
    }
}