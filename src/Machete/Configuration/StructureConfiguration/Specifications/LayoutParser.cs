namespace Machete.StructureConfiguration.Specifications
{
    ...
    {
        readonly Parser<TSchema, LayoutMatch<TLayout, TSchema>>[] _parsers;
        readonly ILayoutFactory<TLayout> _factory;

        public LayoutParser(ILayoutFactory<TLayout> factory, IEnumerable<Parser<TSchema, LayoutMatch<TLayout, TSchema>>> parsers)
        {
            _factory = factory;
        ...
        {
            var matches = new List<LayoutMatch<TLayout, TSchema>>(_parsers.Length);
