namespace Machete.Translators
{
    ...
    public class TranslateCreator<TSchema> :
        ICreator<TSchema>
        where TSchema : Entity
    {
        readonly string _name;
        readonly IReadOnlyDictionary<Type, IEntityCreator<TSchema>> _entityTranslators;
        readonly TranslatorObservable<TSchema> _observers;
        ObserverHandle[] _handles;

        public TranslateCreator(string name, IReadOnlyDictionary<Type, IEntityCreator<TSchema>> entityTranslators)
        {
            _name = name;
        ...
        async Task<TranslateResult<TSchema>> TranslateEntity(TranslateContext<TSchema> context, TSchema entity)
        {
            ...
            {
                var translateResult = await translator.Translate(context).ConfigureAwait(false);
            ...
            foreach (var interfaceType in interfaceTypes)
            {
                ...
                {
                    var translateResult = await entityTranslator.Translate(context).ConfigureAwait(false);
                    if (translateResult.IsTranslated)
                        return translateResult;
                }
            }
